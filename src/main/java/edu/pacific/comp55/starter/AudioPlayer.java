...
public final class AudioPlayer {
	...
	private void playSoundWithOptions(String folder, String filename, boolean shouldLoop) {
		...
		if (mPlayer == null || mPlayer.getCycleDuration().lessThanOrEqualTo(mPlayer.getCurrentTime())) mPlayer = createMediaPlayer(folder, filename);
		mPlayer.play();
		if (shouldLoop) mPlayer.setCycleCount(MediaPlayer.INDEFINITE);
	}
	...
	public void setSound(String folder, String filename, double num) {
		Platform.runLater(new Runnable() {
			public void run() {
				MediaPlayer sound = findSound(folder, filename);
				if (num <= 1 && num >= 0) sound.setVolume(num);;
